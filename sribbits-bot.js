...
async function getInitialPrice() {
  ...
  const wsb = "wsb"
  const sribbits_ADDRESS = "0x399e279c814a3100065fceab8cba1ab114805344"
  const sribbits = "sribbits"
  ...
  let sribbitsSGBPrice = (Number(arryPrice[1]._hex) / 10 ** 18).toFixed(5)
  let wsbUSDPrice = (sgbPrice / wsbSGBPrice).toFixed(5)
  let sribbitsUSDPrice = (sgbPrice / sribbitsSGBPrice).toFixed(5)
  console.log(wsb + " USD price is " + wsbUSDPrice)
  console.log(sribbits + " USD price is " + sribbitsUSDPrice)

  clearRoles()
  lastPrice = sribbitsUSDPrice || 0
  let symbol = `${process.env.COIN_ID.toUpperCase()}`
  client.user.setPresence({
    activities: [{
    name: `SGB=${sribbitsSGBPrice} ${symbol}`,
    type: `PLAYING`
    }]
  })

  arrow = mid
  client.guilds.cache.find(guild => guild.id === process.env.SERVER_ID).me.setNickname(`${symbol} ${arrow} ${process.env.CURRENCY_SYMBOL}${sribbitsUSDPrice}`)
...
async function getPrices() {
  ...
  currentPrice = sribbitsUSDPrice || 0
  let symbol = `${process.env.COIN_ID.toUpperCase()}`
  client.user.setPresence({
    activities: [{
